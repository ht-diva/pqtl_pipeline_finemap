...
dry-run:
	source $(CONDA_ENV_DIR)/activate $(CONDA_ENV_NAME) && \
	snakemake --sdm conda --dry-run --profile slurm --snakefile workflow/ld_top_snps.smk  #--until collect_credible_sets
...
run:
	source $(CONDA_ENV_DIR)/activate $(CONDA_ENV_NAME) && \
	snakemake --profile slurm --snakefile workflow/ld_top_snps.smk #--until collect_credible_sets
